#!/usr/bin/env python3
"""
CPE Question Content Scraper v3
Fetches missing content from ZeroJudge and fills QUESTION-*.md files
Uses proper HTML parsing with regex
"""
...
def extract_description(html: str) -> Optional[str]:
    ...
    for section_id in ["problem_thedescription", "problem_description", "problem_desc"]:
        result = extract_section_by_id(html, section_id)
        if result:
            return result

    # Fallback: look for content between 題目敘述 and 輸入說明
    try:
        start_idx = html.find("題目敘述")
        end_idx = html.find("輸入說明")
...
def scrape_zerojudge(zj_id: str) -> Optional[Dict[str, str]]:
    ...
    # Try to extract by ID first
    description = extract_section_by_id(html, "problem_thedescription")
    if not description:
...
def main():
    """Main execution"""
    print("🚀 CPE Question Content Scraper v3")
