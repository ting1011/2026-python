...
PRIORITY_3 = {
    "week-09": [10409, 10415, 10420, 10642],
    "week-10": [10783],
    "week-12": [10812, 10908],
...
def extract_section(html: str, section_name: str) -> Optional[str]:
    ...
    try:
        # Pattern 1: Look for section header followed by content in divs
        pattern = rf"{section_name}[^<]*(?:<[^>]*>)*\s*<div[^>]*>(.*?)</div>"
        ...
        # Pattern 2: Try alternative structure
        pattern2 = rf"{section_name}[^<]*<[^>]*>([^<]+(?:<[^>]*>[^<]+)*)"
        ...
        print(f"  ⚠️  Parse error for {section_name}: {e}")
        return None
...
def scrape_zerojudge(zj_id: str) -> Optional[Dict[str, str]]:
    ...
    description = extract_section(html, "題目敘述")
    input_desc = extract_section(html, "輸入說明")
    output_desc = extract_section(html, "輸出說明")
...
def main():
    """Main execution"""
    print("🚀 CPE Question Content Scraper v2")
