...
import csv                    # CSV 檔案序列化器
import io                     # 提供 StringIO 類別
import platform               # 偵測作業系統平台
import zipfile                # 讀取壓縮檔案
from pathlib import Path      # 路徑管理

import matplotlib.pyplot as plt  # 繪圖基礎套件
import pandas as pd             # 表格數據操作
import seaborn as sns           # 高級統計視覺化
...
#
# 1) 改畫「各系所」熱力圖：
#    提示：df.groupby(['學年', '系所']).size() 然後 pivot()
#    sns.heatmap(pivot_by_dept, annot=True, fmt='d', cmap='coolwarm')
#    cmap 可選擇其他色譜（如 'Blues', 'Greens', 'YlOrRd' 等）
#
# 2) 加一張圓餅圖：114 學年學院占比
#    提示：pivot[pivot['學年']==114] 等等全部14年的資料
#    fig, axes = plt.subplots(1, 2, ...) 中提增幅傳（變成 1, 3 或另 3 欄）
#    ax = axes[2]，襯重圓餅標註：
#    pd.DataFrame(pivot[...]['by_dept'].most_common()).plot(kind='pie', y=1, ax=ax)
#
# 3) 把年度 x 軸改成「109學年」~「114學年」字串：
#    提示：pivot['學年'] = pivot['學年'].astype(str) + '學年'
#    axes[0].set_xticklabels(pivot['學年'].unique(), rotation=45)
#
# 4) 觸格 ♥ 護位：找出「人數逐年下降最明顯」的學院
#    提示：同 A07 的手法：檔 by_dept Counter 按年份排成一個時間序列 (109, 110, ..., 114)
#    計算首尾人數差異（檢測上升趨勢）
