...
實現一個字符串或數字處理演算法。根據題目的具體定義，可能涉及字符替換、編碼轉換、字符統計等操作。需要正確理解題目規則並高效實現相應的轉換邏輯。

## 輸入說明

輸入包含多個測試案例。每個案例包含待處理的字符串或數字，可能還包含處理方法的參數說明。輸入以EOF結束。

## 輸出說明

輸出經過指定轉換後的結果。如果是字符串轉換，輸出轉換後的字符串；如果是統計問題，輸出統計結果；結果格式按題目要求。
